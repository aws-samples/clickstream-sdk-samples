  ...
  <Layout>
    <div class="container">
      <div class="gridwrapper">
        ...
        <div class="position-relative featured-product-column d-none d-md-block">
          <div class="featured-products">
            ...
            >
              <div>
                <img :src="product.imagePath" alt="" class="mb-2 img-fluid w-100"/>
                <div class="product-name">
                  {{ product.name }}
                </div>
                <div class="container">
                  <div class="row">
                    <div class="col" v-bind:class="{'discounted': product.discounted}">{{
                        formatPrice(product.price)
                      }}
                    </div>
                    <div class="col font-weight-bold" v-if="product.discounted">
                      {{ formatPrice(discountProductPrice(product.price)) }}
                    </div>
                  </div>
                </div>
              </div>
      ...
      <RecommendedProductsSection
          ...
          class="mt-5"
      >
        <template #heading>Compare similar items
          <DemoGuideBadge v-if="demoGuideBadgeArticle" :article="demoGuideBadgeArticle"
...
export default {
  ...
  methods: {
    async getProducts(productIds) {
      ...
      for (const product of productIds) {
        try {
          ...
          data.imagePath = getProductImageUrl(data)
          data["itemId"] = data["id"];
          products.push(data);
        } catch (e) {
          console.log(e)
    ...
    async getRelatedProducts() {
      ...
      let {data} = await ProductsRepository.getProduct(this.activeProductId);

<template>
  <div>
    <!--    <router-link v-if="!user" to="/auth" class="user-dropdown-button login-button btn">Sign In</router-link>-->
    <button @click="signInRandomUser" v-if="!user" class="user-dropdown-button login-button btn">Sign In</button>

    <button
        v-if="user"
        id="navbarDropdown"
        :class="{ 'user-dropdown-button text-left text-lg-right': true, username: !user.persona }"
        data-toggle="dropdown"
        aria-haspopup="true"
        aria-expanded="false"
    >
      <template v-if="user.persona">
        <div class="shopper">Shopper:</div>

        <div>
          <div class="shopper-detail">{{ username }} - {{ user.age }} years - {{ gender }}</div>
          <div class="shopper-detail">{{ formattedPreferences }}</div>
        </div>
      </template>

      <template v-else>{{ username }}</template>
    </button>

    <div v-if="user" class="dropdown-menu px-3" aria-labelledby="navbarDropdown">
      <a href="#" @click="switchShopper" class="dropdown-item">
        <div class="dropdown-item-title">
          Switch shoppers
        </div>
        <div>Select another shopper with different shopping preferences</div>
      </a>

      <div class="dropdown-divider"></div>

      <router-link to="/orders" class="dropdown-item">
        <div class="dropdown-item-title">Orders</div>
        <div>View orders placed by the current shopper profile</div>
      </router-link>

      <div class="dropdown-divider"></div>

      <button class="dropdown-item dropdown-item-title" @click="signOut">Sign Out</button>
    </div>
  </div>
</template>

<script>
import {mapState, mapActions} from 'vuex';

import {Modals} from '@/partials/AppModal/config';
import localUsersRepository from "@/repositories/localUsersRepository";
import {AnalyticsHandler} from "@/analytics/AnalyticsHandler";
import AmplifyStore from "@/store/store";
import router from "@/router";

export default {
  name: 'UserDropdown',
  computed: {
    ...mapState({
      user: (state) => state.user,
      username() {
        // first_name can be an empty string - should be validated against in the future
        if (!this.user) return null;
        if (!this.user.first_name) return this.user.username;
        return this.user.first_name;
      },
      gender() {
        if (!this.user || !this.user.gender) return null;

        switch (this.user.gender) {
          case 'M':
            return 'Male';
          case 'F':
            return 'Female';
        }

        throw new Error('Gender not accounted for');
      },
      formattedPreferences() {
        if (!this.user || !this.user.persona) return null;

        return this.user.persona
            .split('_')
            .map((pref) =>
                pref
                    .split(' ')
                    .map((word) => [word[0].toUpperCase(), ...word.slice(1)].join(''))
                    .join(' '),
            )
            .join(', ');
      },
    }),
  },
  methods: {
    ...mapActions(['openModal']),
    switchShopper() {
      this.openModal(Modals.ShopperSelect);
    },
    async signInRandomUser() {
      // 生成随机用户的逻辑
      const now = new Date()
      let storeUser = await localUsersRepository.getRandomUser()
      storeUser.last_sign_in_date = now.toISOString()
      storeUser.sign_up_date = now.toISOString()
      await AnalyticsHandler.identify(storeUser)
      // Fire sign in and first time sign up events.
      AnalyticsHandler.userSignedUp(storeUser)
      AnalyticsHandler.userSignedIn(storeUser)
      AmplifyStore.commit('setUser', storeUser);
      this.$store.commit('setUser', storeUser);
      localUsersRepository.saveCurrentUser(storeUser)
    },
    signOut() {
      AmplifyStore.dispatch('logout');
      AnalyticsHandler.clearUser()
      if (router.currentRoute.path !== '/') router.push({ path: '/' })
    },
  },
};
</script>

<style scoped>
.user-dropdown-button {
  border: none;
  background: none;
  font-size: 0.85rem;
  line-height: 1.15rem;
}

.login-button {
  border-color: var(--grey-600);
  font-size: 1rem;
}

.shopper {
  font-weight: bold;
  color: var(--blue-600);
}

.shopper-detail {
  font-size: 0.6rem;
  max-width: 170px;
  white-space: nowrap;
  overflow: hidden;
  text-overflow: ellipsis;
}

.username {
  font-size: 1rem;
}

@media (min-width: 992px) {
  .shopper-detail {
    max-width: 260px;
    font-size: 0.8rem;
  }
}

.dropdown-menu {
  max-width: 350px;
}

.dropdown-item {
  white-space: normal;
}

.dropdown-item-title {
  color: var(--blue-600);
}

.dropdown-item:active .dropdown-item-title,
.dropdown-item-title:active {
  color: var(--white);
}
</style>
